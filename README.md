...
建议把这个 skill 视为**手动显式调用优先**的初始化工具，而不是默认在每个新线程里自动接管工作的入口。
...
- 当用户明确要初始化项目时，再使用这个 skill
- 如果只是刚打开一个新线程，不要默认自动进入初始化流程
- 如果发现项目状态缺失但用户尚未明确要求初始化，优先提示用户手动运行 `$project-init`
- 项目已经初始化后，后续常规开发应回到 `using-superpowers` 等日常工作流，而不是再次运行 `project-init`
